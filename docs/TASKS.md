...
- [ ] Re-evaluate B0 (`base_06b`, `base_17b`) with the extended evaluate.py, replacing the runs from commit 79fc74b (decision 38). `base_06b` on data v1.3 done: `runs/base_06b/metrics.json` at commit a1dc2bf, identical per split to the evaluation at 0a0c25e; `base_17b` pending.
...
- Acceptance: runs `sft_06b` and `sft_17b` (run names per the task 1.7 spec; previously `sft_clinc_v1_*`) complete on Kaggle, metrics on all test splits written for both. Training 70.5 min for 0.6B on data v1.3 (1378 steps, micro-batch 8 x 4) and an expected 3 to 3.5 hours for 1.7B (not yet measured); evaluations about 45 min for 0.6B and an expected 1.5 hours for 1.7B, so 1.7B runs in two sessions (docs/KAGGLE.md section 7).
