...
- [ ] `pyproject.toml` with the package `jevmark`, dev extras (pytest), optional extras (serve, baselines).
- [ ] `Makefile` with the targets listed in CLAUDE.md.
- [ ] `configs/base.yaml` with backbone id, max_tokens, LoRA settings, seed, run_name.
- [ ] `tests/conftest.py` that builds a tiny random Qwen3 model (2 layers, hidden 64, vocab from the real Qwen3 tokenizer) for CPU tests.
...
### 1.2 encode.py
- [ ] `encode(request, tokenizer, max_tokens) -> Encoded(input_ids, slot_positions, letter_ids, question_ids)` producing the exact text in API_SPEC section 4.
...
- Acceptance: `tests/test_encode.py` checks the rendered text against a golden string for a three-question request, checks slot positions point at the last token of `Answer:`, and checks that changing option order changes letter assignment but not the set of letter ids.
...
- Acceptance: `tests/test_systemone.py` round-trips the three-question example from API_SPEC through the tiny model and validates the response shape field by field.
...
- Acceptance: `make data` runs end to end; answer-letter histogram in train is close to uniform after shuffling; no held-out intent appears in train or valid (assert in a test).

### 1.6 metrics.py and evaluate.py
- [ ] Metrics: accuracy, macro-F1, ECE (15 equal-width bins on top-1 confidence), Brier (multi-class), NLL, reliability diagram data, coverage-vs-accuracy curve at thresholds 0.0 to 1.0 step 0.05 using the confidence field.
...
- [ ] `scripts/evaluate.py --ckpt <run or 'base'> --splits ...` writes `runs/<run_name>/metrics.json` and reliability plots.
- [ ] Latency: batch-1 wall clock per request on GPU, median over 200 requests, recorded in metrics.json.
- Acceptance: `evaluate.py --ckpt base` on Kaggle produces the frozen-base baseline B0 on all test splits. About 0.5 GPU hours.
