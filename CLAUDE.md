...
- `.gitignore`: `/data/`, `/runs/*/adapter/`, `/runs/*_limit*/` (smoke runs), weight files (`*.safetensors`, `*.bin`, `*.pt`), `.env`, caches. Never a bare `data/` pattern, which would also match `jevmark/data/`.
